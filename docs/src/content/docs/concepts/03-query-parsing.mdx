...
Emitted for the boolean operators `AND` / `OR` / `NOT` (uppercase only) and grouping parens `(` / `)`:

```ts
interface OperatorSegment {
  type: 'operator'
  op: 'and' | 'or' | 'not' | 'open' | 'close'
  start: number
  end: number
}
```

Turn a flat segment stream into a nested tree with `parseExpression`. See [Boolean Operators](/concepts/05-boolean-operators/).
